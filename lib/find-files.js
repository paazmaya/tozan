...
/**
 * Find and filter files under the given directory.
 *
 * @param {string} directory  Root directory in which images should be
 * @param {boolean} ignoreDotFiles Ignore files and directories that begin with a dot
 *
 * @returns {Array} List of files
 */
const findFilterFiles = (directory, ignoreDotFiles) => {
...
const findFiles = (directory, options) => {
  const items = findFilterFiles(directory, options.ignoreDotFiles);
  let files = [];
...
module.exports._canAccessFile = canAccessFile;
module.exports._findFilterFiles = findFilterFiles;
